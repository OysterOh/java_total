...
//		Inter i1 = new Inter();  (x) Cannot instantiate the type Inter
//		인터페이스는 객체를 생성할 수 없는 추상적인 개념이다
		...
		/*
		 - 다형성은 상속이 전제되어야 하는 것이 원칙이지만 예외로 인터페이스와 클래스간의
		   구현 관계에서도 다형성 발생을 허용한다. 
		   인터페이스 변수 = new 객체(); 
		 */
		...
		/*
		 - 인터페이스의 다형성도 앞에서 배운 클래스의 다형성처럼 정보가 없다면 
		   메서드 호출이 불가능하기 때문에 형 변환이 필요하다
		 - 구현하는 클래스가 서로 다른 인터페이스들을 동시에 구현하고 있다면
		   구현하는 클래스들끼리 서로 즉시 형변환이 가능하다.   
		 */
		((Inter2)it1).method2();
