...
public class Calculator {
	...
	/*
	 - 계산기마다 색깔이 각각 다를 수 있기 때문에
	   color같은 변수는 데이터를 공유시키지 말고, 객체별로 따로 관리해야 한다.
	   
	 - 반면 pi같은 원주율은 계산기마다 값이 동일해야 하기 때문에
	   static을 붙여서 데이터를 공유하게 하는 것이 유리하다.   
	 */
	String color;
	static double pi = 3.141592;  //모든 객체에 적용되는 값이라면 static
	
	
	/*
	 - 메서드 내부에서 non-static 데이터를 참조하고 있는 메서드는
	   해당 변수의 정확한 위치(객체의 주소값)을 알려줘야 하기 때문에
	   정적 메서드(static)로 선언하기가 부적절하다. 
	 */
	
	public void paint(String color) {
		System.out.println("계산기에 " + color + "색을 칠한다.");
		this.color = color;
	}
