...
public class StrBuilderExample {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 - String 클래스는 아주 많이 쓰이고 좋은 기능들이 많지만 메모리를 과소비하는 단점이 있다.
		 - String 객체는 처음 초기화된 데이터에 변화를 주어야 하는 상황에서
		   기존 객체를 활용하지 않고 새로운 객체를 계속해서 생성한다.
		   
		 # StringBuilder, StringBuffer(자바 5버전부터 사용 가능)
		 
		 - String  클래스 단점으로 인한 메모리 과부하 및 속도가 느려지는 현상을 개선하기 위해
		   StringBuilder가 새롭게 추가되었다.
		...
		sb.insert(6, "my "); 
		System.out.println(sb);  //hello my world
		
		
		//특정 인덱스 범위의 문자열을 교체하는 메서드: replace(begin, end, 문자열)
		//끝범위는 미만으로 인식된다.
		sb.replace(6, 8, "your");
		System.out.println(sb);  //hello your world
